 ...
 */
 __device__ void spotrf_tile(float* t_A)
 {
    ...
    for(int k{0};k<TILE_SIZE;k++){
        ...
        {
            t_A[(tx+1)*TILE_SIZE + k]/= t_A[k*TILE_SIZE + k];  // if(tx>=1)
        }
        __syncthreads();
 
        if(ty<=tx && tx<TILE_SIZE - 1 && ty<TILE_SIZE - 1 && ty >= k)
        {
            t_A[(tx+1)*TILE_SIZE + (ty+1)]-= t_A[(tx+1)*TILE_SIZE + k]*t_A[(ty+1)*TILE_SIZE + k]; // if(tx>=1 && ty>=1) //or ty>=k+1
        }
        __syncthreads();
    }
 }
 ...
 * implmenting triangular solve on tile t_A2 using t_A1 
 */
