 ...
 __global__ void left_looking_kernel(float *g_in, int N)
 {
    ...
    for(int i{0};i<no_of_tiles;i++){
        ...
        for(int j{0};j<no_of_tiles;j++){
            ...
            rA1 = &s_current_panel[i*no_of_tiles * TILE_SIZE * TILE_SIZE + j*TILE_SIZE * TILE_SIZE];
    ...
    __syncthreads();    
     ...
     for(int i=0; i<no_of_tiles; i++)
     {
         ...
         for(int j=0; j<i; j++)
         {
             ...
             //rA1 = &s_current_panel[i*no_of_tiles * TILE_SIZE * TILE_SIZE + j*TILE_SIZE * TILE_SIZE];
             rA2 = &s_current_panel[i*no_of_tiles * TILE_SIZE * TILE_SIZE + i*TILE_SIZE * TILE_SIZE];
             ...
             for(int k=i+1; k<no_of_tiles; k++)
             {
                 ...
                 rA1 = &s_current_panel[k*no_of_tiles * TILE_SIZE * TILE_SIZE + j*TILE_SIZE * TILE_SIZE];
                 ...
                 // sgemm
                 //rA1 = &s_current_panel[i*no_of_tiles * TILE_SIZE * TILE_SIZE + j*TILE_SIZE * TILE_SIZE];
                 rA2 = &s_current_panel[k*no_of_tiles * TILE_SIZE * TILE_SIZE + j*TILE_SIZE * TILE_SIZE]; //(k,j)
                 rA3 = &s_current_panel[k*no_of_tiles * TILE_SIZE * TILE_SIZE + i*TILE_SIZE * TILE_SIZE]; //(k,i)
         ...
         //rA1 = &s_current_panel[i * TILE_SIZE * TILE_SIZE];
           rA1 = &s_current_panel[i*no_of_tiles * TILE_SIZE * TILE_SIZE + i*TILE_SIZE * TILE_SIZE];
         spotrf_tile(rA1);
         __syncthreads();
         ...
         {
             // trsm
             rA2 = &s_current_panel[k*no_of_tiles * TILE_SIZE * TILE_SIZE + i*TILE_SIZE * TILE_SIZE];
         ...
         {
             rA1 = &s_current_panel[i*no_of_tiles * TILE_SIZE * TILE_SIZE + k*TILE_SIZE * TILE_SIZE];
             ...
             if(j >= i)
             {
                 if(j == i)         // representing the tile on which spotrf will be carried out
                 {
                     for(int k=0; k<i; k++)         // k iterates over tiles left of (i,i) tile
                     {
                         ...
                         rA2 = &s_current_panel[j*no_of_tiles * TILE_SIZE * TILE_SIZE + k*TILE_SIZE * TILE_SIZE];
                 ...
                 else
                 {
                     ...
                     rA3 = &s_current_panel[j*no_of_tiles * TILE_SIZE * TILE_SIZE + i*TILE_SIZE * TILE_SIZE];
                     ...
                     rA2 = &s_current_panel[j*no_of_tiles * TILE_SIZE * TILE_SIZE + i*TILE_SIZE * TILE_SIZE]; 
